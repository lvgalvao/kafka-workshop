...
def criar_transacao(contador):
    mensagem = {
        ...
        'endereco': str(fake.street_address() + ' | ' + fake.city() + ' | ' + fake.country_code()),
        'cadastro_em': str(fake.date_time_this_month()),
        'id_plataforma': str(random.choice(['Mobile', 'Laptop', 'Tablet'])),
        'mensagem': 'transacao feita pelo usuario {}'.format(str(uuid.uuid4().fields[-1]))
    }
    return mensagem
...
while True:
    ...
    print('mensagem enviada para o kafka com id de sequência {}'.format(contador))
    time.sleep(2)
